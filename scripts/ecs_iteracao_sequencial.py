...
benchmark = Benchmark("ecs_iteracao_sequencial")
...
def getName(name):
    ...
    if "Freyr" not in name:
        return split[0]

    return f"{split[0]}_Bloco{split[2]}"
...
fig = px.line(
    ...
    y="Tempo(ms)",
    color="Cenário",
    title="Tempo de execução de iterações sincronas",
    text="Tempo",
    markers=True,
)
fig.update_traces(textposition="bottom right")
fig.write_html(Path("../resources/ecs_iteracao_sequencial_tempo_execucao.html"))
...
fig = px.line(
    ...
    range_y=[0, 1000],
    color="Cenário",
    title="Taxa de atualização de iterações sincronas",
    text="Quadros",
    markers=True,
)
fig.update_traces(textposition="bottom right")
fig.write_html(Path("../resources/ecs_iteracao_sequencial_fps.html"))
